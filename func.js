...
const getComponentsRegistry = function (packagePath) {
	...
	if (registry.components) {
		return OracleBotLib.ComponentRegistry.create(
			registry.components,
...
if (componentsRegistry && componentsRegistry.getComponents().size > 0) {
	...
	throw new Error(
		'Unable to process component registry because no components were found in package: ' +
			packagePath
	);
}
...
const _handle = function (input, ctx) {
	...
	if (!botsFnPath) {
		...
		if (!componentName) {
			throw new Error(
				'The component name is missing from the header ' +
					BOTS_FN_PATH_HEADER +
					': ' +
					botsFnPath
			);
