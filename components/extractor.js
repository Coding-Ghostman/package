...
Leaves cannot be applied on weekends, so adjust the dates to the nearest weekday. If a weekend date is mentioned, move it to the following Monday.
...
function generateExtractionPreamble(existingData, userProfile) {
    ...
    INSTRUCTIONS: WHILE HANDLING DATES CONSIDER ONLY THE WORKING DAYS i.e. MONDAY TO FRIDAY. IF THE USER IS ASKING TO TAKE LEAVE ON A WEEKEND, ADJUST IT TO THE FOLLOWING MONDAY.
    ...
    IMPORTANT: If the user is asking for leave on a weekend, adjust it to the following Monday. Don't ignore the request.
